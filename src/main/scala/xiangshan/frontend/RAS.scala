...
class RAS(implicit p: Parameters) extends BasePredictor {
  ...
  class RASStack(rasSize: Int, rasSpecSize: Int) extends XSModule with HasCircularQueuePtrHelper {
    ...
    when (io.redirect_valid && io.redirect_isCall) {
      ...
      // clear current top wriXSPerfAccumulateteBypass if doing redirect
  ...
  val update = io.update(dupForRas).bits
  val updateMeta = io.update(dupForRas).bits.meta.asTypeOf(new RASMeta)
  val updateValid = io.update(dupForRas).valid
